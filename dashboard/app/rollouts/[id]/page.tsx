      ...
      <div className="grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-6">
        ...
        <div className="space-y-6">
          ...
          <section className="rounded-lg border border-gray-200 bg-white p-5">
            ...
            {r.judged_sample_count === 0 && (r.sample_count_candidate ?? 0) > 0 && (
              <div className="mb-4 flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2.5">
                ...
                <p className="text-[12.5px] leading-relaxed text-amber-800">
                  <span className="font-semibold">Quality is not being measured.</span>{" "}
                  No responses have been scored by the LLM judge, so the scores below
                  reflect only basic health checks (non-empty, no error, not a refusal)
                  and cannot tell the candidate apart from the baseline. This rollout
                  will hold rather than advance until judging resumes — check that the
                  evaluator&rsquo;s OpenAI API key is valid.
                </p>
              </div>
            )}
